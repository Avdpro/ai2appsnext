...
//***************************************************************************
{
	...
	//***********************************************************************
	{
		...
		//---------------------------------------------------------------------------
		webDriveApp.setPermission = async function(opts = {}) {
			const nameRaw = String(opts.name || "").trim();
			const stateRaw = String(opts.state || "").trim().toLowerCase();
			const origin = String(opts.origin || "").trim();
			const userContext = String(opts.userContext || "default").trim();
			const context = String(opts.context || "").trim();
			if (!nameRaw) throw new Error("setPermission: missing permission name");
			if (!stateRaw) throw new Error("setPermission: missing state");
			const descriptor = { name: nameRaw };

			const candidates = [];
			if (origin) {
				candidates.push({ descriptor, state: stateRaw, origin, userContext });
				candidates.push({ descriptor, state: stateRaw, origin });
			}
			if (context) {
				candidates.push({ descriptor, state: stateRaw, context });
			}
			// Compatibility fallback for different implementations.
			if (origin) {
				candidates.push({ permission: descriptor, value: stateRaw, origin, userContext });
				candidates.push({ permission: descriptor, value: stateRaw, origin });
			}
			if (context) {
				candidates.push({ permission: descriptor, value: stateRaw, context });
			}
			if (!candidates.length) {
				candidates.push({ descriptor, state: stateRaw });
				candidates.push({ permission: descriptor, value: stateRaw });
			}

			let lastErr = null;
			for (const params of candidates) {
				try {
					return await this.sendCommand("permissions.setPermission", params);
				} catch (err) {
					lastErr = err;
				}
			}
			throw lastErr || new Error("setPermission failed");
		};
