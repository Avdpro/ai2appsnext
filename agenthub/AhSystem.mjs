...
let AhSystem,ahSystem;
{
	...
	//-----------------------------------------------------------------------
	ahSystem.listAgentNodes=async function(){
		let nodeMap,nodeVO,node,staticVO,list;
		await this.scanAgents(false);
		nodeMap=new Map();
		list=this.staticNodes;
		for(nodeVO of list){
			nodeMap.set(nodeVO.name,{...nodeVO,active:false,external:false});
		}
		list=Array.from(this.nodeMap.values());
		for(node of list){
	...
	//-----------------------------------------------------------------------
	ahSystem.listMcpTools=async function(){
		let nodes,tools,node,agents,agent;
		nodes=await this.listAgentNodes();
		tools=[{
			name:"agenthub.ping",
			...
			inputSchema:{
				type:"object",
				properties:{
					echo:{type:["string","number","object","array","boolean","null"]}
				},
		...
		for(node of nodes){
			agents=node.agents||[];
			for(agent of agents){
				let toolName=`${node.name}.${agent}`;
				tools.push({
					name:toolName,
					description:`${node.description||"Agent node tool"} (${node.name})`,
					inputSchema:{
						type:"object",
						properties:{
							input:{description:"Input passed to agent script",type:["string","number","object","array","boolean","null"]},
							prompt:{description:"Alias of input",type:["string","number","object","array","boolean","null"]},
	...
	//-----------------------------------------------------------------------
	ahSystem.callMcpTool=async function(toolName,args){
		let dotPos,nodeName,agentPath,node,input,timeoutMs,language;
		args=args||{};
		if(toolName==="agenthub.ping"){
			return {
				ok:true,
				tool:"agenthub.ping",
				echo:("echo" in args)?args.echo:null,
				time:new Date().toISOString()
			};
		}
		dotPos=toolName.indexOf(".");
		if(dotPos<=0 || dotPos>=toolName.length-1){
			throw Error(`Invalid tool name "${toolName}". Expect "<node>.<agent>".`);
		}
		nodeName=toolName.slice(0,dotPos);
		agentPath=toolName.slice(dotPos+1);
		input=(args.prompt!==undefined)?args.prompt:((args.input!==undefined)?args.input:args);
		timeoutMs=Math.max(1000,args.timeoutMs||300000);
		language=args.language||this.configJSON?.language||"EN";

		node=await this.startAgentNode(nodeName,null,{});
		if(!node){
			throw Error(`Node "${nodeName}" not found or failed to start.`);
		}
		return await node.execAgentOnce(agentPath,input,{
			callTimeoutMs:timeoutMs,
			sessionOptions:{language:language}
		});
	};
	...
	ahSystem.setup=async function(router,apiMap){
		...
		apiMap['AhListAgentNodes']=async function(req,res){
			let reqVO;
			reqVO = req.body.vo;
			res.json({ code: 200, nodes:await self.listAgentNodes()});
		};
