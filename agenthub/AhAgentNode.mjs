...
let AhAgentNode,ahAgentNode;
{
	...
	//-----------------------------------------------------------------------
	ahAgentNode.execAgentOnce=async function(agentPath,input,options){
		let sessionId,readyPms,execPms,readyRes,readyRej,execRes,execRej;
		let startTimeout,execTimeout;
		options=options||{};
		if(!this.nodeWS){
			throw Error(`AgentNode "${this.name}" is not connected.`);
		}
		sessionId=`McpSession-${Date.now()}-${nextMcpSessionId++}`;
		readyPms=new Promise((resolve,reject)=>{
			readyRes=resolve;
			readyRej=reject;
		});
		execPms=new Promise((resolve,reject)=>{
			execRes=resolve;
			execRej=reject;
		});
		const startTimeoutMs=options.startTimeoutMs||15000;
		const callTimeoutMs=options.callTimeoutMs||300000;
		let promptUsed=false;
		let waitBlockIdx=0;
		startTimeout=setTimeout(()=>{
			readyRej(Error(`CreateSession timeout for "${this.name}" (${startTimeoutMs}ms).`));
		},startTimeoutMs);
		execTimeout=setTimeout(()=>{
			execRej(Error(`ExecAgent timeout for "${this.name}" (${callTimeoutMs}ms).`));
		},callTimeoutMs);
		const tempSession={
			sessionId:sessionId,
			isExecuting:true,
			sendToClient:async function(msg,vo){
				// No UI client in MCP mode, ignore display/debug pushes.
				return true;
			},
			callClient:async function(msg,vo){
				switch(msg){
					case "AskChatInput":
						if(promptUsed){
							return "";
					...
					case "AddWaitBlock":
						return `mcp-wait-${waitBlockIdx++}`;
					case "AskUpward":
						return "";
					case "FindAgent":
						return null;
					default:
						// Non-interactive MCP path: ignore UI-only calls.
						return null;
				}
			},
			...
			OnStartError:async function(){
				clearTimeout(startTimeout);
				readyRej(Error(`CreateSession failed for "${sessionId}".`));
			},
			execAgentEnd:async (result,error)=>{
		...
		this.sessionMap.set(sessionId,tempSession);
		try{
			await this.nodeWS.send(JSON.stringify({
				msg:"CreateSession",
				sessionId:sessionId,
			...
			if(this.nodeWS){
				try{
					await this.nodeWS.send(JSON.stringify({msg:"EndSession",sessionId:sessionId}));
				}catch(err){
					//Ignore session cleanup send error
				}
			}
		}
	};
