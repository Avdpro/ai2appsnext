...
const inPagePrompt = function (text, opts = {}) {
	...
	return new Promise((resolve) => {
		try {
			...
			if (!host) return resolve((hasMenu || edit) ? null : (returnBoolean ? false : { ok: false, reason: "no-root" }));
			...
			// Guard against page-level CSS that forces opacity/filter on prompt text.
...
#__ai2apps_prompt_root__ [data-ai2apps-dim="1"] {
  opacity: 0.45 !important;
}
`;
			root.appendChild(guardStyle);
			...
			dlg.style.opacity = "1";
			dlg.style.filter = "none";
			dlg.style.mixBlendMode = "normal";
			dlg.style.backdropFilter = "none";
			dlg.style.webkitBackdropFilter = "none";
			dlg.style.isolation = "isolate";
			...
			const updateOkEnabled = () => {
				...
				if (enabled) okBtn.removeAttribute("data-ai2apps-dim");
				else okBtn.setAttribute("data-ai2apps-dim", "1");
			...
			const makeMenuRow = (it, idx, groupName) => {
				...
				if (disabled) {
					...
					row.setAttribute("data-ai2apps-dim", "1");
...
const inPageTip=function(text, opts = {}) {
	function ai2appsTipDismiss(idOrAll) {
		...
		const closeEl = (el) => {
			...
			el.style.setProperty("--ai2apps-tip-opacity", "0");
	...
	return new Promise((resolve) => {
		try {
			if (!host) return resolve({ ok: false, id: "", action: "error", reason: "no-root" });
			...
			if (!root) {
				...
				const guardStyle = doc.createElement("style");
				guardStyle.textContent = `
#__ai2apps_tip_root__,
#__ai2apps_tip_root__ * {
  filter: none !important;
  mix-blend-mode: normal !important;
}
#__ai2apps_tip_root__ [data-ai2apps-tip='1'] {
  opacity: var(--ai2apps-tip-opacity, 1) !important;
}
#__ai2apps_tip_root__ [data-ai2apps-tip='1'] * {
  opacity: 1 !important;
}
#__ai2apps_tip_root__ [data-ai2apps-tip-allow-blur='1'] {
...
`;
			...
			if (!tip) {
				...
				tip.setAttribute("data-ai2apps-tip-allow-blur", "1");
				...
				tip.style.gap = "10px";
				tip.style.opacity = "var(--ai2apps-tip-opacity, 1)";
				tip.style.setProperty("--ai2apps-tip-opacity", "0");
			...
			requestAnimationFrame(() => {
				tip.style.setProperty("--ai2apps-tip-opacity", String(opacity));
			...
			(function ensureTipHoverFader(rootEl) {
				...
				const apply = () => {
					...
					for (const el of tips) {
						...
						el.style.setProperty("--ai2apps-tip-opacity", inside ? String(hov) : String(normal));
					}
				};
