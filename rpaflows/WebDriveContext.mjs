...
//***************************************************************************
{
	...
	aaWebDriveContext.isClosed=async function(){
		...
		try {
			...
			// If getTree fails because context/frame no longer exists, treat as closed.
			const errText = String(error?.message || error || "").toLowerCase();
			if (
				errText.includes("no such ")
				|| errText.includes("not found")
				|| errText.includes("browsing context")
				|| errText.includes("no such frame")
			) {
				return true;
			}
...
//***************************************************************************
{
	...
	aaWebDriveContext.waitForDownloadBegin=async function(opts){
		...
		const matchContext=opts?.matchContext===true;
		const contextId=this.context;
		const eventA="downloads.downloadStarted";
		const eventB="browsingContext.downloadStarted";
		waitFunc=(message)=>{
			let call;
			const msgContext=message?.context||message?.browsingContext?.context||message?.browsingContext||"";
			if(matchContext && msgContext && msgContext!==contextId){
				return;
			}
			if(timer){
				clearTimeout(timer);
			...
			this.webDrive.off(eventA,waitFunc);
			this.webDrive.off(eventB,waitFunc);
			call=callback;
			if(call){
				...
				callerror=null;
				call({
					suggestedFilename:message.suggestedFilename||message.filename||dl.suggestedFilename||"",
					file:message.file||message.filepath||dl.file||"",
					url:message.url||dl.url||"",
					context:msgContext||"",
				});
			}
		}
		return new Promise((resolve,reject)=>{
			callback=resolve;
			callerror=reject;
			this.webDrive.on(eventA,waitFunc);
			this.webDrive.on(eventB,waitFunc);
			if(timeout>0){
				timer=setTimeout(()=>{
					let call;
					this.webDrive.off(eventA,waitFunc);
	...
	aaWebDriveContext.waitForDownloadEnd=async function(opts){
		...
		const matchContext=opts?.matchContext===true;
		const contextId=this.context;
		const eventA="downloads.downloadFinished";
		const eventB="browsingContext.downloadFinished";
		waitFunc=(message)=>{
			let call;
			const msgContext=message?.context||message?.browsingContext?.context||message?.browsingContext||"";
			if(matchContext && msgContext && msgContext!==contextId){
			...
			if(call){
				...
				callerror=null;
				call({
					status:message.status||message.result||dl.state||"",
					filepath:message.filepath||message.file||dl.file||"",
					url:message.url||dl.url||"",
					context:msgContext||"",
				});
			}
